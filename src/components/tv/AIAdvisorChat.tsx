...
const CANNED = [
  "Based on current viewership, Nessma TV leads prime time with a 22% share.",
  "Peak zapping happens around 20:00 — schedule ads just before for higher retention.",
  "For 25-34 women, El Hiwar Ettounsi has the strongest match between 19:30 and 20:30.",
  "Digital billboards in Tunis Lac 1 deliver the best cost-per-impression this week.",
  "Mosaique FM dominates the Greater Tunis area with consistent morning peaks.",
];

export function AIAdvisorChat() {
  const [open, setOpen] = useState(false);
  const [messages, setMessages] = useState<Msg[]>([
    { role: "bot", text: "Hi! I'm your media strategy assistant. Ask me anything about TV audiences, channels or ad timing." },
  ]);
      ...
      {open && (
        <div className="fixed bottom-24 right-6 z-[450] flex h-[520px] w-[360px] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-xl border border-border bg-card shadow-2xl animate-fade-in">
          ...
          <div className="flex-1 space-y-3 overflow-y-auto p-4">
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[80%] rounded-lg px-3 py-2 text-sm ${m.role === "user" ? "bg-primary text-primary-foreground" : "border border-border bg-background/60 text-foreground"}`}>
                  {m.text}
                </div>
              </div>
            ))}
          ...
          >
            ...
            />
            <button type="submit" className="flex h-9 w-9 items-center justify-center rounded-md bg-primary text-primary-foreground hover:opacity-90">
